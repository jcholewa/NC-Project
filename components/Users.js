...
const Users = props => {
  const { users, onSelectUser, currentUser, navigation, city } = props;
  const _refresh = () => {
  ...
  return users.length >= 2 ? (
    <PTRView onRefresh={_refresh}>
      <ScrollView style={cardStyles.scrollView}>
        ...
        >{`Scroll down to see users nearby in ${city}`}</Text>
        {users.map(
          user =>
            user[0] !== currentUser.uid && (
              ...
              >
                ...
                >
                  {user[1].distance === 0 ? "Less than 100" : user[1].distance}m
                  away
                </Text>
              </TouchableOpacity>
            )
    ...
    <PTRView
      onRefresh={_refresh}
      style={{ backgroundColor: colorSettings.usersNoNearbyUsers }}
    >
      <View style={[{ flex: 1 }]}>
        <Text
          style={[
