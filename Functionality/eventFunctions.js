...
const getEventCoords = async locationString =>
  Expo.Location.geocodeAsync(locationString);
...
const getNearbyEvents = async cb =>
  firebase.auth().onAuthStateChanged(currentUser => {
    if (currentUser) {
      ...
      return usersRef
        ...
        .then(snapshot =>
          Promise.all([
            eventsRef.where("datetime", ">", Date.now()).get(),
            snapshot.data()
          ]).then(([querySnapshot, userObj]) => {
            const eventsData = [];
            ...
            return Promise.all([
              ...
              const sortedEvents = [...nearbyEvents].sort(
